...
def main():
    ...
    for epoch in range(begin_epoch+1, cfg.TRAIN.END_EPOCH+1):
        ...
        if (epoch % cfg.TRAIN.VAL_FREQ == 0 or epoch == cfg.TRAIN.END_EPOCH) and rank in [-1, 0]:
            ...
            msg = 'Epoch: [{0}]    Loss({loss:.3f})\n' \
                      ...
                      'Time: inference({t_inf:.4f}s/frame)  nms({t_nms:.4f}s/frame)'.format(
                          epoch,  loss=total_loss,  ll_seg_acc=ll_segment_results[0],ll_seg_iou=ll_segment_results[1],ll_seg_miou=ll_segment_results[2],
        ...
        if rank in [-1, 0]:
            if epoch % cfg.SAVE_NUM ==0 and epoch > 0:
